...
/*  macro constants used for customization of mcu-libc

    TLSF_MALLOC_FL_INDEX_BITS and TLSF_MALLOC_SL_INDEX_BITS are compile time parameters
    that define maximum amount of memory manageable by TLSF and number of subdivisions
    between 2^n and 2^(n+1) that at the end define amount of trashed memory when TLSF
    must allocate a block larger then requested.

    In this note TLSF_MALLOC_FL_INDEX_BITS is shorted to FLIB and TLSF_MALLOC_SL_INDEX_BITS
    is shorted to SLIB.

    FLSF_MEM is internal memory used by TLSF itself and it can be calculated as:
    FLSF_MEM = (FLIB * SLIB + 6) * sizeof(void*) + (FLIB + 1) * sizeof(int);
    On 16-bit MCU this is roughly 2 * (FLIB * (SLIB + 1) + 7) bytes.
    On 32-bit MCU this is roughly 4 * (FLIB * (SLIB + 1) + 7) bytes.
    FLIB    SLIB    16-bit  32-bit
    13      2       92      184
    14      2       98      196
    15      2       104     208
    16      2       110     220
    ...
    Minimal block size is 3 * sizeof(void*) which is 8 bytes on 16-bit MCU (block size rounded up to 4)
    and 12 bytes on 32-bit MCU.

    Maximal block size is 2^FLIB - FLSF_MEM.

    So, FLIB should be defined depending on memory available to TLSF:
    FLIB    Memory
    12      4KB
    13      8KB
    14      16KB
    ...
    SLIB will define number of subdivisions between 2^n and 2^(n+1). If SLIB is big we will have
    fine grain subdivisions for every class of sizes and memory trashing will be low, but overhead
    of memory used by TLSF itself will be higher.
    For MCU using up to 32KB of memory good choice for SLIB is 2 or 3.
*/
// how many bits are used for addressing memory
#ifndef TLSF_MALLOC_FL_INDEX_BITS
#define TLSF_MALLOC_FL_INDEX_BITS   15
#endif // TLSF_MALLOC_FL_INDEX_BITS

// how many bits is used to calculate subdivisions between 2^n and 2^(n+1)
#ifndef TLSF_MALLOC_SL_INDEX_BITS
#define TLSF_MALLOC_SL_INDEX_BITS   2
#endif // TLSF_MALLOC_SL_INDEX_BITS
...
enum tlsf_public
{
    /* log2 of number of linear subdivisions of block sizes.
       number of linear subdivisions = 2 ^ SL_INDEX_COUNT_LOG2 */
    // SL_INDEX_COUNT_LOG2 = 5,
    SL_INDEX_COUNT_LOG2 = TLSF_MALLOC_SL_INDEX_BITS,
};
...
enum tlsf_private
{
    ...
    */
    // FL_INDEX_MAX = 32,
    FL_INDEX_MAX = TLSF_MALLOC_FL_INDEX_BITS,
#else
    // FL_INDEX_MAX = 30,
    // For MCU that have only 30KB of contiguous memory we need only 14 or 15 bits
    FL_INDEX_MAX = TLSF_MALLOC_FL_INDEX_BITS,
#endif
...
#define tlsf_cast(t, exp)    ((t) ((size_t)(exp)))
