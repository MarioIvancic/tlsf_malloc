...
#define MALLOC_AUTOINIT() if(!malloc_pool) malloc_pool = tlsf_create(_heap, TLSF_MALLOC_MEMSIZE)
...
/*  Allocates memory for an array of nmemb elements of size bytes each, returning 
    a pointer to the beginning of the block. The content of the newly allocated block of memory
    is initialized to 0.
    If the function failed to allocate the requested block of memory,
    or if size is zero a null pointer is returned.
*/
void* calloc (size_t nmemb, size_t size)
{
    ...
    size_t s = size * nmemb;
    void* p = tlsf_malloc(malloc_pool, s);
    if(p != NULL) memset(p, 0, s);
    return p;
}
